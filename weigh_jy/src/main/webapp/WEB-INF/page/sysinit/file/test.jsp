	...
	Ext.onReady(function(){
		...
		*/
		var formcols = [{id:'userid',fieldset:'详细',
			items: [
					...
					{fieldLabel:'部门',xtype: 'estcombos',hiddenName:'sysDept.deptid', valueField:'deptid', displayField:'deptname', storeurl:'est/sysinit/sysdept/SysDept/getAllDepart'},
					new Ext.form.DateField({fieldLabel:'生日',name:'birthday',format:'Y-m-d',altFormats:'Y-m-d|Y-m-d H:i:s'}),
					{fieldLabel:'性別', xtype: 'estcombos', width: 80, hiddenName:'sex', elms: [['男','男'],['女','女']]},
		...
		*/
		var gridPanel = {
			...
			items: [
				{xtype: 'estgrid', id:'grid', storeurl: '<%=basePath%>est/sysinit/sysuser/SysUser/getUserList', colstype: colsType, region: 'center'}, searchPanel
			]
		};
		...
		*/
		var treePanel = {xtype: 'esttree',isctx: true, rootTxt:'所有部门', id:'treepanel', region:'west',loaderurl: '<%=basePath%>est/sysinit/sysdept/SysDept/getDepartTree',
				ctx: {url: '<%=basePath%>est/sysinit/sysdept/SysDept', method: 'Dept', ctxitems:[{id:'add',text:'添加'},{id:'modify',text:'修改'},{id:'delete',
					text:'刪除'},{id:'cus',text:'自定义',fun:function() {alert('custom')}}],
					formitems:
		...
		*/
		...
		Ext.getCmp('searchpanel').addRefObjs(Ext.getCmp('grid'));
			...
			var cmnew =		new Ext.grid.ColumnModel([{dataIndex:'username', header: '用户名', width: '30', sortable: false},
						{dataIndex:'sysDept.deptname', header: '所在部门',width: '40', sortable: true}
					])
		var g = Ext.getCmp('grid');
		g.reconfigure(g.store,cmnew);
