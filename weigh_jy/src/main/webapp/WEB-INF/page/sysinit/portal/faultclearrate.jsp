...
<script type="text/javascript">
	...
	*/
	...
	var chartpanel = {
			xtype:'estchartpanel',
			region:'fit',
			chartUrl:'<%=basePath%>est/sysinit/portal/FaultPortal/getFaultClearRateForChart',
			id:'chartpanel',
			title:'图表',
			autoScroll :true
		}
	...
	// 搜索字段
	var searchFields=[	   
		'时间:',{xtype:'datefield',id:'startTime',format:'Y-m-d',width:100},
        '-'	  ,{xtype:'datefield',id:'endTime',format:'Y-m-d',width:100},
        '部门:',{xtype:'estpropcombos',id:'deptName',propertycode:'QXJXDW',emptyText:'请选择上一级',width:100},
        '类别:',{xtype:'estpropcombos',id:'faultType',propertycode:'QXLB',readOnly:true,width:80,emptyText:'请选择上一级'}
      ];
  	// 搜索字段映射
  	var searchMapping={'startTime':'startTime','endTime':'endTime','deptName':'deptName','faultType':'faultType'};
	*/
	
	var colsType = {
				cols: [
						{dataIndex:'dept', header: '消缺部门',width: 40},
						{dataIndex:'faultClearTotal', header: '已消缺数', width: 40},
						{dataIndex:'needClearAmount', header: '需要消缺数',width: 40},
						{dataIndex:'faultClearRate',header:'消缺率(%)',width:40,renderer:function(v){
	...
	var gridpanel = {
		...
		items: [
			{xtype: 'estgrid', id:'grid',storeurl: '<%=basePath%>est/sysinit/portal/FaultPortal/getFaultClearRateForGrid', colstype: colsType, region: 'center'}
		]
	};
