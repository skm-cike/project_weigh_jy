...
//					{dataIndex: 'water', name:'water', header:'水分',width:60},
    	...
    	var formcols = [
			{fieldset:'过磅数据详细信息',id:'id',
				items: [
				  {fieldLabel:'磅单号', name:'pounds_number'},
				   {fieldLabel:'车号',name:'vehicle_no'},
					{fieldLabel:'车重',name:'vehicle_weight'},
					{fieldLabel:'毛重',name:'gross_weight'},
					{fieldLabel:'扣吨(扣重)',name:'de_weight'},
					 <%if("fenmeihui".equals(request.getParameter("pz")) || "shigao".equals(request.getParameter("pz"))) {%>
                    ...
                    {fieldLabel:'机组',name:'jizu',id:'jizu_comb',hiddenName:'jizu',displayField:'propertyname',valueField:'propertyname',xtype:'estpropcombos',propertycode:'FMHJZ'},
                    <%} %>
			        ...
			        {fieldLabel:'单价(不填写自动匹配)',name:'unit_price'},
			         <%}%>
					 pagevendorCombox,
					{fieldLabel:'毛重时间',name:'gross_time',xtype:'estdatetime',format:'Y-m-d H:i',altFormats:'Y-m-d H:i|Y-m-d H:i:s',width:130,allowBlank: false},
					{fieldLabel:'皮重时间',name:'vehicle_time',xtype:'estdatetime',format:'Y-m-d H:i',altFormats:'Y-m-d H:i|Y-m-d H:i:s',width:130,allowBlank: false},
			        {fieldLabel:'毛重司磅人',name:'weighman'},
			        {fieldLabel:'皮重司磅人',name:'tareweightman'},
                   ...
                   {name:'companyname',hidden:true,hideLabel:true}
		        ]
		      }
		   ];
