...
/** * 将10亿以内的阿拉伯数字转成汉字大写形式 * @author liu chang *  */
public class CnUpperCaser {  
	...
	/*	if(floatPart.length()>0){      
		 ...
		 */
		if(floatPart.length()>0){      
			int number=getNumber(floatPart.charAt(0));
			sb.append(cnNumbers[number]+"角");  
			if(floatPart.length()>1){
				int number1=getNumber(floatPart.charAt(1)); 
				sb.append(cnNumbers[number1]+"分");  
			}
